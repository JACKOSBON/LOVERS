...
// ============================================================
//  API HEALTH CHECK — FIXED (Only checks API availability)
// ============================================================
let apiStatus = {};
...
async function checkAPIHealth() {
    ...
    // Simple health check — just check if API returns a response
    const healthChecks = {
        bitcoin: 'https://blockchain.info/ticker',
        ethereum: 'https://api.etherscan.io/api?module=stats&action=ethprice&apikey=YourApiKeyToken',
        bsc: 'https://api.bscscan.com/api?module=stats&action=bnbprice&apikey=YourApiKeyToken',
        polygon: 'https://api.polygonscan.com/api?module=stats&action=maticprice&apikey=YourApiKeyToken',
        solana: 'https://api.solscan.io/',
        tron: 'https://api.trongrid.io/',
        monero: 'https://xmrchain.net/'
    };
    
    for (const [chain, url] of Object.entries(healthChecks)) {
        try {
            const resp = await fetch(url, { signal: AbortSignal.timeout(5000) });
            results[chain] = resp.ok ? '✅ Working' : '❌ Dead (status: ' + resp.status + ')';
        } catch (e) {
            results[chain] = '❌ Dead (' + e.message + ')';
        }
    }
    
    apiStatus = results;
    sendLog(`📊 API Health: ${JSON.stringify(results)}`, 'info');
}
...
async function loop() {
    ...
    while (true) {
        ...
        stats.speed = elapsed > 0 ? Math.round(stats.generated / elapsed) : 0;
    }
}
